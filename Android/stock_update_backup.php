...
/* 	     $Fullbarcode = "Rexin_1006";
	 ...
	 $BatchName=$obj["batchName"];
		 ...
		 $sqlselect = "SELECT a.Barcode,b.ProductId,c.CategoryId,d.SubCategoryId,'".$StockType."','".$Quantity."',e.UomId,f.LocationId,g.UnitId, h.id,i.BatchId
 ...
 inner join unitdetails as g
 inner join users as h
inner join batchdetails as i
 
 ON 1=1 WHERE a.Fullbarcode = '".$Fullbarcode."' and b.Itemcode = '".$Itemcode."' and c.CategoryName='".$CategoryName."' and d.SubCategoryName = '".$SubCategoryName."'
 and e.UomSubType = '".$UomSubType."' and f.LocationName ='".$LocationName."' and g.UnitName='None' and h.username ='".$UserName."' and i.batchname ='".$BatchName."'";
...
echo "no";die();
        ...
        //on below line we are selecting the course detail with below id.
     $sql = "insert into stockdetails (`Barcode`, `BatchId`,`ProductId`, `CategoryId`, `SubCategoryId`, `StockType`, `Quantity`, `UomId`, `LocationId`, `UnitId`,`id`) 
	 Values(".$row['Barcode'].",".$row['BatchId'].",".$row['ProductId'].",".$row['CategoryId'].",".$row['SubCategoryId'].",'".$StockType."',".$Quantity.",".$row['UomId'].",".$row['LocationId'].",".$row['UnitId'].",".$row['id'].")";
	 //echo $sql;die();
 $response = array();
	 if($conn->query($sql)){
         $response['error'] = false;
