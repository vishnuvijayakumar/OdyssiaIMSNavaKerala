<?php
  ...
  $all_batchs = find_all('batchdetails');
  $all_challans = find_all('challandetails');
...
<?php
 if(isset($_POST['add_stock'])){
   $req_fields = array('barcode','product-name','product-category','batch','product-subcategory','product-quantity','UOM','stock-type','product-location' );
   validate_fields($req_fields);
   if(empty($errors)){
    ...
    $p_code  = remove_junk($db->escape($_POST['product-name']));
     ...
     $p_challan  = remove_junk($db->escape($_POST['challanno']));
     $userid = current_user();
     $p_userid = $userid['id'];
     //echo $p_uom;die();
     
      $batchdetails= find_by_id_custom("batchdetails",$p_batch,"BatchId");
      if(($p_stock_type!='In - First Entry' || $p_stock_type!='In - Excess' ) && (!empty($batchdetails)) && isset($batchdetails['BatchDate'])) {
          ...
          if(!empty($checkstockbatchstatus) && isset($checkstockbatchstatus['BatchShortName'])) {
              $errors="Given product has older batch stock so kindly check and use the older stock...!";
     ...
     if(isset($_POST['product-quantity-limit']) && ($p_stock_type == "Out - Plan No" || $p_stock_typ == "Out - Chelan No" || $p_stock_type == "Out - Additional") && (remove_junk($db->escape($_POST['product-quantity-limit'])<$p_qty_converted))) {
    ...
    //   $media_id = '0';
     ...
     $query .=" Barcode,BatchId,ProductId,CategoryId,SubCategoryId,StockType,Quantity,UomId,LocationId,UnitId,PlanNo,ChallanId,id";
     $query .=") VALUES (";
     $query .=" '{$b_code}',{$p_batch},'{$p_code}', '{$p_cat}', '{$p_subcat}', '{$p_stock_type}', '{$p_qty}','{$p_uom}', '{$p_loc}', '{$p_unit}', '{$p_planno}','{$p_challan}', '{$p_userid}'";
     $query .=")";
          ...
          <form method="post" action="Stock_Add.php" class="clearfix">
            ...
            <?php } endforeach; ?>
              ...
              <div class="form-group">
                <div class="row">
                  ...
                  <select class="form-control" name="batch">
                    ...
                    <?php  foreach ($all_batchs as $bat): ?>
                      <option value="<?php echo $bat['BatchId']; ?>" <?php if(isset($Stocks['BatchId']) && ($Stocks['BatchId']==$bat['BatchId'])){ echo "Selected";  }?>>
                        <?php echo $bat['BatchShortName'] ?></option>
                    <?php endforeach; ?>
                    </select>
                  </div>
                  <div class="col-md-4">
              ...
              <div class="form-group">
                <div class="row">
                  ...
                  <div class="input-group">
                     ...
                     <input type="text" class="form-control" name="planno" placeholder="Plan No">
              ...
              <div class="form-group">
                <div class="row">
                  <div class="col-md-6">
                  <select class="form-control" name="challanno">
                      <option value="">Select Challan</option>
                    <?php  foreach ($all_challans as $chal): ?>
                      <option value="<?php echo $chal['ChallanId']; ?>">
                        <?php echo $chal['ChallanName'] ?></option>
                    <?php endforeach; ?>
                    </select>
                  </div>
                </div>
